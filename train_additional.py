...
# (train_model 함수는 수정할 필요 없이 동일합니다)
def train_model(
    model,
    train_iterator,
    ...
    # Create optimizer
    optimizer = optim.AdamW(model.network.parameters(), lr=lr, weight_decay=0.1)
...
def main(args):
    ...
    excluded_keys = ['device', 'batch_size', 'num_iterations', 
                     ...
                     'load_checkpoint'] # [수정] load_checkpoint는 모델 생성 인자가 아님
    ...
    if args.load_checkpoint:
        ...
        try:
            ...
            # ---
            # [핵심 수정] "state_dict" 키가 있는지 확인하고, 그 안의 값을 로드합니다.
            # ---
            if 'state_dict' in checkpoint:
                model.load_state_dict(checkpoint['state_dict'])
                print("Model weights loaded successfully (from 'state_dict' key).")

                
        except Exception as e:
            print(f"Error loading checkpoint: {e}")
            print("Training from scratch instead.") # <--- 여기가 실행된 것
...
if __name__ == "__main__":
    ...
    # [수정] 추가 학습을 위한 인자 2개 추가
    parser.add_argument("--load_checkpoint", type=str, default=None,
                        help="Path to checkpoint file to load and continue training (e.g., results/11-12-074744/final_model.pt)")
    parser.add_argument("--empirical_ratio", type=float, default=0.75,
                        help="Ratio of Empirical (Flow-Matching) to Bootstrap (Self-Consistency) targets. Default 0.75.")
